...
from . import mainWindow as mainWindow
from . import processing.activation as pr_activation
from . import processing.heatmap as pr_heatmap
from . import processing.origami_ms as pr_origami
from . import processing.spectra as pr_spectra
from . import readers.io_document as io_document
from . import readers.io_text_files as io_text
from . import readers.io_waters_raw as io_waters
from . import unidec as unidec
from .config import OrigamiConfig as config
from .dialogs import (panelCalibrantDB, panelHTMLViewer, panelNotifyNewVersion,
                      panelSelectDocument)
...
#                 print(ind[0], tD)
    ...
    def on2DTextFileFcn(self, path=None, e=None):
        ...
        # Add to table
        color = self.view.panelMultipleText.on_check_duplicate_colors(
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            for (filepath, filename) in zip(pathlist, filenames):
                ...
                if isempty(xAxisLabels) or isempty(yAxisLabels):
                    ...
                    msg = "Missing x/y-axis labels for %s! Consider adding x/y-axis to your file to obtain full functionality." % (
                        filename)
    ...
    def on_extract_2D_from_mass_range(self, extract_type="all"):
        ...
        for row in range(tempList.GetItemCount()):
            ...
            if filename == '':
                self.onThreading(
                    None, ('File name column was empty. Using the current document name instead', 4), action='updateStatusbar')
...
#                                                      driftscope_path=self.config.driftscopePath)
    ...
    def onExtractMSforEachCollVoltage(self, evt):
        ...
        if (self.config.ms_mzStart == None or self.config.ms_mzEnd == None
            ...
            self.onThreading(
                None, ('Please fill in appopriate fields: MS start, MS end and MS bin size', 3), action='updateStatusbar')
            return
...
#             self.view.panelControls.onUpdateParams()
            ...
            self.view.panelPlots.on_plot_2D_data(
                data=[imsData2D, xlabels, 'Scans', ylabels, 'Drift time (bins)', self.docs.colormap])
            tend = time.clock()
    ...
    def get_overlay_document(self):
        ...
        if self.documentsDict[self.currentDoc].dataType == 'Type: Comparison':
            document = self.documentsDict[self.currentDoc]
            self.onThreading(None, ('Using document: ' + document.title.encode('ascii', 'replace'), 4),
                             action='updateStatusbar')
        else:
            ...
            if len(docList) == 0:
                ...
                if self.currentDoc == None:
                    ...
                    return
                document = self.documentsDict[self.currentDoc]
                self.onThreading(None, ('Using document: ' + document.title.encode('ascii',
                                                                                   'replace'), 4), action='updateStatusbar')
    ...
    def on_overlay_1D(self, source, plot_type):
        ...
        if add_data_to_document:
            if self.documentsDict[self.currentDoc].dataType == 'Type: Comparison':
                self.docs = self.documentsDict[self.currentDoc]
                self.onThreading(None, ('Using document: ' + self.docs.title.encode('ascii',
                                                                                    'replace'), 4), action='updateStatusbar')
            else:
                docList = self.checkIfAnyDocumentsAreOfType(type='Type: Comparison')
                if len(docList) == 0:
    ...
    def on_open_multiple_ML_files(self, open_type, pathlist=[]):
        ...
        for i, file_path in enumerate(pathlist):
            ...
            if path is None:
                ...
                color = convertRGB255to1(self.view.panelMML.on_check_duplicate_colors(
...
# # ---
    ...
    def onChangeChargeState(self, evt):
        ...
        self.currentDoc, selectedItem, selectedText = self.view.panelDocuments.topP.documents.enableCurrentDocument(
            getSelected=True)
...
#         self.view.panelControls.colorBtn.SetBackgroundColour(newColour)
    ...
    def OnBuildCCSCalibrationDataset(self, evt):
        ...
        if tempList.GetItemCount() == 0:
            self.onThreading(
                None, ('Cannot build calibration curve as the calibration list is empty. Load data first.', 4), action='updateStatusbar')
        try:
            self.currentDoc = self.view.panelDocuments.topP.documents.enableCurrentDocument()
        except:
            return
...
#             if cmap == '' or cmap is None: cmap = 'viridis'
    ...
    def process2Ddata2(self, zvals=None, labelsX=None, e=None, mode='2D'):
        ...
        if isempty(threshold) or threshold == None or threshold == '':
            pass
        else:
...
#             self.view.updateRecentFiles()
    ...
    def getCurrentDocumentPath(self):
        ...
        if self.currentDoc == 'Current documents':
            ...
            selectedItemParent, selectedItemParentText = self.view.panelDocuments.topP.documents.getParentItem(
                ...
                try:
                    if evt.GetId() == ID_CHECK_VERSION:
                        dialogs.dlgBox(exceptionTitle='ORIGAMI',
                                       exceptionMsg='You are using the most up to date version %s.' % (
                                           self.config.version),
                                       type="Info")
                except:
                    pass
