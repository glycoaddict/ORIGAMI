...
#                                               bitmap=self.icons.iconsLib['panel_ccs_16']))
        ...
        self.Bind(wx.EVT_MENU, self.presenter.on_open_multiple_ORIGAMI_files, id=ID_openMultipleORIGAMIRawFiles)
        ...
        self.Bind(wx.EVT_MENU, self.presenter.on_open_ML_1D, id=ID_open1DIMSFile)
        self.Bind(wx.EVT_MENU, self.presenter.on_open_ML_2D, id=ID_open2DIMSFile)
...
#         self.Bind(wx.EVT_MENU, self.onSequenceEditor, id=ID_sequence_openGUI)
    ...
    def on_import_configuration_file(self, evt, onStart=False):
        ...
        if not onStart:
            if evt.GetId() == ID_openConfig:
                ...
                self.presenter.onThreading(None, ("Imported configuration file: {}".format(config_path), 4), action='updateStatusbar')
    ...
    def on_export_configuration_file(self, evt, verbose=True):
        if isinstance(evt, int): evtID = evt
...
#             return
    ...
    def onToolbarPosition(self, evt):
        ...
        evtID = evt.GetId()
        # Try hiding
        try: self._mgr.GetPane(self.mainToolbar_vertical).Hide()
        except: pass
        
        try: self._mgr.GetPane(self.mainToolbar_horizontal).Hide()
        except: pass
    ...
    def makeToolbar(self):
        ...
        if self.config._windowSettings['Toolbar']['orientation'] in ['top', 'bottom']:
            ...
            self.mainToolbar_horizontal.AddSeparator()
            self.mainToolbar_horizontal.AddTool(ID_extraSettings_general_plot, self.icons.iconsLib['panel_plot_general_16'], shortHelpString="Settings: General plot")
            ...
            self.mainToolbar_horizontal.AddTool(ID_extraSettings_violin, self.icons.iconsLib['panel_violin_16'], shortHelpString="Settings: Violin panel")
            self.mainToolbar_horizontal.AddTool(ID_extraSettings_general, self.icons.iconsLib['panel_general2_16'], shortHelpString="Settings: Extra panel")                    
            self.mainToolbar_horizontal.AddSeparator()
            ...
            self.mainToolbar_vertical.AddSeparator()
            self.mainToolbar_vertical.AddTool(ID_extraSettings_general_plot, self.icons.iconsLib['panel_plot_general_16'], shortHelpString="Settings: General plot")
            ...
            self.mainToolbar_vertical.AddTool(ID_extraSettings_violin, self.icons.iconsLib['panel_violin_16'], shortHelpString="Settings: Violin panel")
            self.mainToolbar_vertical.AddTool(ID_extraSettings_general, self.icons.iconsLib['panel_general2_16'], shortHelpString="Settings: Extra panel")
            self.mainToolbar_vertical.AddSeparator()
    ...
    def onPaneOnOff(self, evt, check=None):
        ...
        if isinstance(evt, int):
            ...
            if evt == "document": evtID = ID_window_documentList
            elif evt == "ion": evtID = ID_window_ionList
            elif evt == "text": evtID = ID_window_textList
            elif evt == "ccs": evtID = ID_window_ccsList
            elif evt == "mass_spectra": evtID = ID_window_multipleMLList
            elif evt == "dt": evtID = ID_window_multiFieldList
            elif evt == "log": evtID = ID_window_logWindow
    ...
    def extract_from_plot_1D(self, xvalsMin, xvalsMax, yvalsMax, currentView=None, currentDoc=""):
        ...
        if currentDocument == "Current documents": return
...
#         if not self.panelParametersEdit.IsShown()
    ...
    def onPlotParameters(self, evt):
        ...
        elif evt.GetId() == ID_extraSettings_violin: kwargs = {'window':'Violin'}
        elif evt.GetId() == ID_extraSettings_general: kwargs = {'window':'Extra'}
        elif evt.GetId() == ID_extraSettings_general_plot: kwargs = {'window':'General'}
